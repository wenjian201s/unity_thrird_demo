...
public class CombatControllerBase : MonoBehaviour  // 战斗控制基类，继承 MonoBehaviour
{
    ...
    public bool IsAttackAnimationActive => IsCurrentAttackAnimationState(); // 当前 Animator 是否正在播放/过渡到攻击动画，用于外部阻止跳跃等动作
    ...
    private bool IsCurrentAttackAnimationState()
    {
        if (animator == null)
            return false;

        if (IsAttackStateInfo(animator.GetCurrentAnimatorStateInfo(0)))
            return true;

        return animator.IsInTransition(0) && IsAttackStateInfo(animator.GetNextAnimatorStateInfo(0));
    }

    private bool IsAttackStateInfo(AnimatorStateInfo stateInfo)
    {
        if (stateInfo.IsTag("Attack") || stateInfo.IsTag("GSAttack") || stateInfo.IsTag("KatanaAttack"))
            return true;

        if (currentComboList == null)
            return false;

        string comboName = currentComboList.TryGetComboName(currentComboIndex);
        return !string.IsNullOrEmpty(comboName) && stateInfo.IsName(comboName);
    }
